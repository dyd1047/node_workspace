...
Get : 가져오다(서버로부터 컨텐츠를 가져올 때 사용되는 요청 방식)
        ...
        또한 편지봉투에 데이터를 전송하면 편지지에 비해 전송할 수 있는 데이터양의 한계가 있다.

        주용도) 다른 URL로 링크를 통해 파라미터 전송할 때

Post : 보내다는 의미 (클라이언트가 서버에 데이터를 전송할 때 사용하는 방식)
         HTTP 프로토콜의 body를 통해 데이터를 전송하기 때문에 편지지에 데이터를 실어서 보내는
         것과 같다. 봉투안에 담아서 보내기 때문에 데이터가 노출되지 않는다.
         특징) 보낼 수 있는 데이터량의 한계가 없다.
         용도) 노출되면 위험한 데이터 전송시 사용(로그인요청, 회원가입 등)
                 파일(사진, 동영상등) 등을 서버에 전송시 이용
...
var server=http.createServer(function(request, response){
    ...
    console.log("Pass : ", paramJson.pass);
        ...
        // body로 전송된 데이터는 url 방식만으로는 해결이 안된다.
        // post 방식으로 전달된 데이터를 받기 위한 이벤트를 감지해보자
        request.on("data", function(param){
            // var urlJson=querystring.parse(request.url, true); // true 매개변수를 넘기면, 파라미터 정보를 json으로 변환
