...
var fs = require("fs"); // File system 모듈, 파일을읽거나 쓸 수 있는 모듈
...
// http 메카니즘이다. 만일 응답을 안하면?? 클라이언트는 무한정 서버의 응답을 기닥리므로
// 무한대기 상태에 빠진다.
// 서버 객체 생성시, 요청정보와 응답정보를 구성할 수 있는 request, response 객체가 매개변수로
// 전달될 수 있다.
var server = http.createServer(function(request, response){
    // request 객체로는 글라이언트의 요청정보를 처리할 수 있고,
    // response 객체로는 클라이언트에게 전송할 응답 정보를 구성할 수 있다.
    console.log("클라이언트의 요청을 받았습니다.");

    // 컨텐츠 전송 (클라이언트의 브라우저가 받게될 내용)
    // HTTP 상태 코드 중 200은 정상 처리를 의미..(즉 서버에서 클라이언트의 요청을 정상적으로
    ...
    // response.writeHead(200, {"Content-Type":"image/png;"}); // 편지봉투 구성하기

    // fs.readFile("../images/mario/m.png", "utf-8", function(error, data){
    //     response.end(data);
    // });
});
...
// 왜? 하나의 os 내에서 수 많은 네트워크 프로그램들간 엉키지 않기 위해
