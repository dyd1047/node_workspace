...
var server=http.createServer(function(request, response){
    ...
    // 클라이언트의 요청에 대한 응답 처리...(html 문서를 주고받음)
    ...
    if(result.pathname=="/login"){
        // console.log("mysql 연동하여 회원 존재여부 확인합니다.");
    ...
    }else if(result.pathname=="/chook"){
        console.log("초옥이 입니다..");
        response.end("초옥이 불러올게요.");
        ...
        fs.readFile("./loginForm.html", "utf-8", function(error, data){
            if(error){
                ...
                // HTTP 프로토콜로 데이터를 주고 받을  떄는, 이미 정해진 규약이 있으므로 눈에 보이지 않는
                // 수많은 설정 정보값들을 서버와 클아이언트간 교환한다.
                response.end(data);
            }
        });
    }
    
    // 전체 url 중에서도 uri 만을 추출해보자
    // 따라서 전체 .url을 해석해야 한다. 해석은 parsing한다고 한다.
