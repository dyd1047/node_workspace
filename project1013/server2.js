...
var server = http.createServer(function(request, response){
    // response.end("this text is form my server");
    ...
    // 장점? json은 객체표기법이므로, 데이터들을 .찍고 마치 객체다루듯 접근할 수 있기 때문에
    // 직관성이 높아진다. 즉 다루기 쉬워진다. (배열보다 났다.)
    console.log("url 분석결과 : ", parseObj);

    response.writeHead(200, {"Content-Type":"text/html;charset=utf-8"});
    if(parseObj.pathname=="/member/registForm"){ // 회원가입 양식
        fs.readFile("./registForm.html", "utf-8", function(error, data){
            if(error){
                console.log("읽기 실패", error)
            }else{
                response.end(data); // html 파일을 읽어들인 결과 문자열을 클라이언트의 응답 정보로 저장
            }
        });
        ...
        con.query(sql, [param.uid, param.password, param.uname, param.phone, param.email, param.receive, param.addr, param.memo], function(error, result, fields){
            if(error){
                ...
                var msg = "<script>";
                msg+="alert('가입성공');";
                msg+="location.href='/member/list';"; // a태그 동일
