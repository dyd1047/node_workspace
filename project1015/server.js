...
var server = http.createServer(function(request, response){
    ...
    }else if(urlJson.pathname=="/animal"){//동물정보를 요청하면..
...
function update(request, response){
    ...
    request.on("data", function(param){
        ...
        // 디버깅용
        // var sql="update member2 set phone='"+postParam.phone+"', email='"+postParam.email+"', addr='"+postParam.addr+"', memo='"+postParam.memo+"', password='"+postParam.password+"', receive='"+postParam.receive+"'";
        // sql+="where member2_id="+postParam.member2_id;
        var sql="update member2 set phone=?, email=?, addr=?, memo=?, password=?, receive=?";
        sql+="where member2_id=?"
        con.query(sql, [
            postParam.phone,
            postParam.email,
            postParam.addr,
    ...
    con.query(sql, function(error, record, fields){
        ...
        if(error){
            ...
            fs.readFile("./animal.ejs", "utf-8", function(err, data){
                if(err){
                    console.log("animal.ejs 읽기실패", err);
                }else{
                    response.writeHead(200, {"Content-Type":"text/html;charset=utf-8"});
                    response.end(ejs.render(data, {
